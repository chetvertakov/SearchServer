...
void TestDocumentsMatching() {
    ...
    // Затем проверяем что при наличии минус-слова возвращается пустой список
    {
        ...
        try {
            ...
            ASSERT_EQUAL_HINT(get<0>(response), get<0>(expected_response),
                              "Empty list should be returned if document contain minus word"s);
        }  catch (...) {
            ASSERT_HINT (false, "Problems with matching correct document"s);
        }
    }
...
void TestComputeAverageRating () {
    ...
    auto found_docs = server.FindTopDocuments("кот"s);
...
void TestFindedDocumentsPredicate () {
    ...
    server.AddDocument(0, "белый кот и модный ошейник"s,  DocumentStatus::ACTUAL, ratings1);
...
void TestFindedDocumentsStatus () {
    ...
    server.AddDocument(1, "пушистый кот пушистый хвост"s, DocumentStatus::BANNED);
    ...
    //проверка на фильтрацию по статусу
    {
        ...
        ASSERT_EQUAL_HINT(found_docs_par.size(), 1u, "Incorrect documents filtering by STATUS"s);
        ASSERT_EQUAL_HINT(found_docs_par.at(0).id, 1, "Incorrect documents filtering by STATUS"s);
    }
    ...
    //проверка на фильтрацию документов с несуществующим статусом
    {
        ...
        auto found_docs_par = server.FindTopDocuments(std::execution::par, "пушистый ухоженный кот"s, DocumentStatus::IRRELEVANT);
        ASSERT_EQUAL_HINT(found_docs_par.size(), 0u, "Incorrect documents filtering by STATUS"s);
    }
}
...
void TestFindedDocumentsRelevance () {
    ...
    server.AddDocument(3, "ухоженный скворец евгений"s,         DocumentStatus::ACTUAL, {1});
    ...
    auto found_docs_par = server.FindTopDocuments(std::execution::par, "пушистый пушистый ухоженный кот"s);
